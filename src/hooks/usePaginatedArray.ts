...
export const usePaginatedArray = ({ apiUrl, step }: PaginatedHookType) => {
  const [data, setData] = useState<any>([]);
  const paginationStep = step;
  const [paginationIndex, setPaginationIndex] =
    useState<number>(paginationStep);
  ...
  const fetchData = () => {
    ...
    if (newpaginationIndex < responseData.length + step) {
      setData([
        ...data,
        ...responseData.slice(paginationIndex, newpaginationIndex),
      ]);
