      ...
      <Box component={"section"} className={`${styles["result-section"]}`}>
        <Grid container spacing={1}>
          {openStates[0] && (
            <>
              <Grid item xl={6} lg={6} md={5} sm={5}>
                <GridView key={1} totalData={meta?.pagination?.total} loading={loading} data={isFilter===null?places:places.filter((item)=>{return item.id===isFilter})} fetchData={fetchPlaces} hasMoreData={hasMoreData} />
              </Grid>
              {/* To-do: map view */}
          ...
          {openStates[1] && (
            ...
            >
              <ListView key={3} loading={loading} data={isFilter===null?places:places.filter((item)=>{return item.id===isFilter})} fetchData={fetchPlaces} hasMoreData={hasMoreData} />
            </Box>
          )}
        </Grid>
      </Box>
