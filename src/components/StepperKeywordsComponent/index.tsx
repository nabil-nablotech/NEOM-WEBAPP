...
export const StepperKeywordsComponent = ({
  ...
  const { addNewItemWindowType, associatedPlaces, associatedEvents } =
    useSelector((state: RootState) => state.searchResults);
  const [currentlyShownList, setCurrentlyShownList] = useState<string[] | []>(
    []
  );
  const [preloadedKeywordsList, setPreloadedKeywordsList] = useState<
    string[] | []
  >([]);
  const [selectAll, setSelectAll] = useState<boolean>(false);

  const apiKeyword: dbIdTypes =
    addNewItemWindowType === "Places"
      ? "place"
      : addNewItemWindowType === "Events"
      ? "event"
      : addNewItemWindowType === "Library"
      ? "library"
      : "media";
    ...
    if (value.length > 0) {
      ...
      newList =
        ...
        refList
          ...
          .filter((item: string) => item.indexOf(value.toLowerCase()) !== -1);
  ...
  useEffect(() => {
    ...
    if (currentKeywordArray.length > 0 && currentlyShownList.length === 0) {
  ...
  const onDeleteKeyWord = (value: any) => {
    ...
    if (!detectLowerCaseStringInArray(value, preloadedKeywordsList)) {
      setCurrentlyShownList((state) =>
        createSelectedFirstList(state.filter((item) => item !== value))
      );
    }
  };
  ...
  const handleSelectAll = () => {
    if (currentlyShownList.length > 0) {
      if (!selectAll) {
        ...
        setCurrentKeywordsArray([]);
        setCurrentlyShownList(
          createSelectedFirstList([...preloadedKeywordsList])
        );
      }
    }
      ...
      <Box component="div">
        ...
        >
          ...
          {currentlyShownList &&
            ...
            currentlyShownList.map((item: string, index: any) => (
              <span key={index}>
                <CustomChip
                  index={index}
                  item={item}
